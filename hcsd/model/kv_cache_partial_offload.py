...
#     def __str__(self):
#         return f"{self.type}:{self.index if self.index is not None else 'None'}"
...
class KVCache:
    ...
    @property
    def dtype(self):
        """Return the dtype of the data tensor."""
...
def initialize_past_key_values(model):
    ...
    print("start_data_m",start_data_m)
    for i in range(config.num_hidden_layers):
        # print(i)
    ...
    for layer_idx, layer_kv in enumerate(past_key_values):
        for kv_idx, kv_cache in enumerate(layer_kv):
            print(f"Layer {layer_idx}, KV {kv_idx}: device={kv_cache.device}, dtype={kv_cache.dtype}, shape={kv_cache.data.shape}")
