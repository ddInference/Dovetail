...
parser.add_argument('--tmpdir', type=str,
                    default='/gf3/home/lzq/DateSet/sharegpt_0_67999_mufp16')
parser.add_argument('--outdir', type=str, default='0')
parser.add_argument('--cpdir', type=str, default='/gf3/home/lzq/EAGLE-train/layer_1_norm_0_deepspeed_test')
...
train_config = {
    ...
    "residual": "true,norm",
    "max_len": 2048,
    "config_path": "/gf3/home/lzq/EAGLE-train/eagle/train/llama_2_chat_7B_config_1.json",
    "b1": 0.9,
    "b2": 0.95,
    "grad_clip": 0.5,
}
...
def getkacc(model, data, max_length=5):
    ...
    for i in range(bs):
        for j in range(sl):
            ...
            single_hidden_states = single_hidden_states
...
for epoch in range(num_epochs):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        model.zero_grad()
        # print("Input dtype:", data["hidden_states"].dtype)
    ...
    if accelerator.is_local_main_process:
        ...
        logdict1 = {"train/epochacc": correct / (total + 1e-5), "train/epochloss": epoch_loss}
        print(logdict1)
    ...
    for batch_idx, data in enumerate(test_loader):
        with torch.no_grad():
            ...
            predict = model(data["hidden_states"].to(rank), input_ids=data["input_ids"].to(rank),
                            attention_mask=data["attention_mask"].to(rank))
    ...
    if accelerator.is_local_main_process:
        for id, i in enumerate(top_3acc):
            print({f'test/top_{id + 1}_acc': i.sum().item() / (total + 1e-5)})
    epoch_loss /= num_batches
    if accelerator.is_local_main_process:
        print('Test Epoch [{}/{}], Loss: {:.4f}'.format(epoch + 1, num_epochs, epoch_loss))
        print('Test Accuracy: {:.2f}%'.format(100 * correct / (total + 1e-5)))
        print({"test/epochacc": correct / (total + 1e-5), "test/epochloss": epoch_loss})
