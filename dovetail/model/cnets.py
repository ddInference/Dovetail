...
def standard_gru_update(hidden_state, token, W_z, W_1, W_2):
    ...
    #w1_silu or w1_silu_norm
    h1 = torch.matmul(hidden_state, W_1)
    h2 = silu(torch.matmul(token,W_2))
    combined = torch.cat((h1, h2), dim=-1)
    z = sigmoid(torch.matmul(combined, W_z))
    h_t = z*h1 + (1-z)*h2
...
class Model(nn.Module):
        ...
        if load_emb:
            ...
            try:
                ...
                with safe_open(os.path.join(path, emb_path),
                    ...
                    ##############################
                    # tensor = tensor_slice[:, :hidden_dim].to(torch.float32, device="cpu")
                    ##############################
            except:
                ...
                # tensor = weights["model.embed_tokens.weight"].to(torch.float32, device="cpu")
                ###############################
            self.embed_tokens.weight.data = tensor
            

        self.input_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.W_z = nn.Parameter(torch.randn( 2*config.hidden_size, config.hidden_size))
        self.W_1 = nn.Parameter(torch.randn( config.hidden_size, config.hidden_size))
        self.W_2 = nn.Parameter(torch.randn( config.hidden_size, config.hidden_size))
        ...
        if load_head:
            ...
            try:
                ...
                with safe_open(os.path.join(path, head_path),
                    ...
                    # tensor = tensor_slice[:, :hidden_dim].float()
                    ##############################
                    tensor = tensor_slice[:, :hidden_dim].to(dtype=torch.float16, device=torch.device("cuda:0"))
                    ##############################
            except:
                with open(os.path.join(path, "pytorch_model.bin.index.json"), "r") as f:
                    index_json = json.loads(f.read())
                    head_path = index_json["weight_map"]["lm_head.weight"]
                # weights = torch.load(os.path.join(path, head_path),, map_location="cpu")
                # tensor = weights["lm_head.weight"].to(torch.float32, device="cpu")
                ##############################
                weights = torch.load(os.path.join(path, head_path), map_location="cpu")
                tensor = weights["lm_head.weight"].to(dtype=torch.float16, device="cuda:0")
                ###############################
    ...
    def forward(
        ...
        # output_quantify = self.model_quantify.generate(input_ids, attention_mask=masks, generation_config=generation_config,output_hidden_states=True,return_dict_in_generate=True)
        ...
        hidden_states = standard_gru_update(
    ...
    ########################
        ...
        hidden_states = hidden_states.to(device='cuda:0', dtype=torch.float16)
        ##################################
