...
def initialize_past_key_values(model):
    ...
    # print("model.model.layers[i].self_attn.q_proj.weight.dtype",model.model.layers[1].self_attn.q_proj.weight.dtype)
    devices=[]
    ...
    for id,i in enumerate(devices):
        if startdevice!=i:
            past_key_values_data = torch.zeros(
                ...
                device=startdevice,
                dtype=model.dtype,
                # dtype = model.model.layers[1].self_attn.q_proj.weight.dtype,
            )
    ...
    # 遍历 past_key_values 并打印每层的键值对数据类型
    # for layer_index, layer_kv in enumerate(past_key_values):
    #     print(f"Layer {layer_index}:")
    #     for kv_index, kv in enumerate(layer_kv):
    #         if isinstance(kv, KVCache):
    #             print(f"  KV {kv_index} dtype: {kv.data.dtype}")  # 打印数据类型
    #         else:
    #             print(f"  KV {kv_index} 不是 KVCache 对象")
